import { MTimeDifferencePipe } from './time-difference.pipe'

describe('MTimeDifferencePipe', () => {
  let pipe: MTimeDifferencePipe
  const start: number = Date.now()
  const end: number = new Date(start).getTime()

  beforeEach(() => {
    pipe = new MTimeDifferencePipe()
  })
  ...
  it('should return a time ago string', () => {
    ...
    // 1 day
    expect(pipe.transform(start - 1000 * 60 * 60 * 24, end)).toEqual(
      '1 day - 00:00:00'
    )
    // < 1 month
    expect(pipe.transform(start - 1000 * 60 * 60 * 24 * 17, end)).toEqual(
      '17 days - 00:00:00'
    )
  })
})
