import { MTimeAgoPipe } from './time-ago.pipe';

describe('MTimeAgoPipe', () => {
  let pipe: MTimeAgoPipe;
  const time: number = Date.now();

  beforeEach(() => {
    pipe = new MTimeAgoPipe();
  })
  ...
  it('should return "Invalid Date" with an invalid date', () => {
    ...
    expect(pipe.transform({}, {})).toEqual('Invalid Date');
    expect(
      pipe.transform('this is not a valid date', 'not a valid date either')
  ...
  it('should return a time ago string', () => {
    ...
    expect(pipe.transform(new Date(time).toString(), time)).toEqual(
      '1 second ago'
    );
    // < 1 minute
    ...
    // 1 day
    expect(pipe.transform(time - 1000 * 60 * 60 * 24, time)).toEqual(
      '1 day ago'
    );
    // < 1 month
  ...
  it('should use current date when reference is not valid', () => {
    expect(pipe.transform(new Date(time), 'not a number')).toEqual(
      '1 second ago'
    );
  })
})
